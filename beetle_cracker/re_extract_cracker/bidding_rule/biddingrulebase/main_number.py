...
def run_pipeline(data_path):
    ...
    for data in tqdm(datas):
        ...
        # print('data: %s ' % data)
        data.pop('招标单位联系方式', '')
        # print('招标联系人: %s ' % data.pop('招标单位联系人', ''))
        ...
        try:
            ...
            data['招标单位联系方式'] = list({i['text'] for i in result})
...
def evaluate(true_data_path, pred_data_path):
    ...
    for url in url2true_data.keys():
        ...
        y_true = set(true_data.get('招标单位联系方式', []))
        y_pred = set(pred_data.get('招标单位联系方式', []))
        if not y_pred:
            y_pred_index = ''
        else:
    ...
    # metrics, miss, error = calc_metrics(true_set, pred_set)
    # print(metrics)
    metrics_df = pd.DataFrame([metrics], index=['招标单位联系方式'])
...
{'precision': 0.8952380526077117, 'recall': 0.806866918160218, 'f1': 0.8487534401996922}
{'precision': 0.8952380526077117, 'recall': 0.806866918160218, 'f1': 0.8487534401996922}
