...
def _init_data(stop_key="([省市]|特别行政区|自治区)$") -> (dict, Matcher):
    ...
    with resource_stream(__package__, 'resources/adcodes.csv') as csv_stream:
    # with open('.\\resources\\adcodes.csv') as csv_stream:
...
def _extract_addrs(sentence, pos_sensitive, umap, truncate_pos=True, new_entry_when_not_belong=False) -> dict:
    ...
    for match_info in matcher.iter(sentence):
        ...
        # 不同省市间, 区名容易出现重复, 所以此处对此进行一次过滤
        if not last_info and not first_adcode and match_info.origin_value.endswith("区") and len(match_info.attr_infos) > 1:
            continue
        ...
        if cur_addr:
            ...
            # 不同省市间, 区名容易出现重复, 所以此处对此进行一次过滤
            if addr.rank == AddrInfo.RANK_COUNTY and addr.name.endswith("区"):
                # addr = addr_ori
                continue
            res = empty_record(pos_sensitive)
            set_pos(res, match_info.get_rank(), match_info.start_index)
