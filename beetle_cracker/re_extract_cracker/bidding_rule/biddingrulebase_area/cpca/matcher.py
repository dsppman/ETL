...
from .special_abbre import special_abbre
...
class Matcher:
    ...
    def _first_add_addr(self, addr_info):
        ...
        # 地址名与简写共享一个list
        share_list = []
        if type(abbr_name) is list:
            for _name in abbr_name:
                self.ac.add_word(_name, (_name, share_list))
        else:
