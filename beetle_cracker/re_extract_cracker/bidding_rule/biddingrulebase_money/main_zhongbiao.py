...
from bidding_rb.pipeline_zhongbiao_money import Pipeline
...
def run_pipeline(data_path):
    ...
    for data in tqdm(datas):
        ...
        try:
            ...
            data['中标金额'] = list({i['money'] for i in result})
            data['message'] = result
...
def evaluate(true_data_path, pred_data_path):
    ...
    for url in url2true_data.keys():
        ...
        y_true = set(true_data.get('中标金额', []))
        y_pred = set(pred_data.get('中标金额', []))
        miss = y_true - y_pred
        error = y_pred - y_true
    ...
    metrics, miss, error = calc_metrics(true_set, pred_set)
    print(metrics)
    metrics_df = pd.DataFrame([metrics], index=['中标金额'])
