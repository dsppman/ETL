...
class Pipeline:
        ...
        for target_keyword in self.target_keywords:
            ...
            if flag:
                for match_result in match_results:
                    ...
                    regex = r'中\s*标\s*人\b|中\s*标\s*机\s*构\b|采\s*购\s*人\b|采\s*购\s*机\s*构\b|甲\s*方\b'
                    ...
                    if bool(match):
                        ...
                        for contact_number_word in self.contact_number_words:
                            ...
                            if bool(m1):
                                ...
                                for contact_number_word in self.contact_number_words:
                                    ...
                                    reject = re.compile(regex).search(body_part[m1.span()[0] - 6:m1.span()[0]])
                                    if bool(m2) and not reject:
                                        flag, phones = self.find_phone(body_part[m2.span()[1]:m2.span()[1] + 30])
                                        if flag:
                                            texts.extend(phones)
                                            result = [{
                                                'text': cell,
                        ...
                        if flag:
                            for number_text in number_texts:
                                flag, phones = self.find_phone(number_text)
                                if flag:
                                    texts.extend(phones)
                                    result = [{
                                        'text': cell,
    ...
    def rule_firstly_table(self, table):
            ...
            for j, cell in enumerate(cells):
                ...
                for target_keyword in self.target_keywords:
                    if self.regex_match(target_keyword, cell)[0]:
                        logger.info(f'rule_firstly_table: {target_keyword} {cell}')
                        match_ij.append((i, j))
        ...
        for i, j in match_ij:
            ...
            for s_cell in search_cells:
                ...
                if flag:
                    flag, phones = self.find_phone(text_numbers[0])
                    if flag:
                        ...
                        for secondly_cell in secondly_cells:
                            flag, phones = self.find_phone(secondly_cell[0][:25])
                            if flag:
                                target_cells.extend(phones)
                                break
    ...
    def find_name(self, text):
        result = []
        text = re.split(r'电\s*话|邮\s*件|邮\s*箱|地\s*址|手\s*机|联\s*系', text)[0]
    ...
    def find_phone(self, text):
        ...
        result = []
        text = re.split(
            "电\\s*话|手\\s*机|方\\s*式|EMAIL|邮\\s*箱|Q\\s*Q|q\\s*q|金\\s*额|价\\s*格|中\\s*标\\s*价",
            text
        )[0]
        search_texts = []
        ...
        if bool(match):
            ...
            for family_name_word in family_name_words:
                if text[span[0]] == family_name_word:
                    ...
                    flag, results = self.find_phone(text[span[1]:25])
                    #{'precision': 0.9305554263117463, 'recall': 0.837499895312513, 'f1': 0.8815738452498193}
                    if flag:
                        final_flag = True
                        name = text[span[0]:span[1]]
                        phones = results
