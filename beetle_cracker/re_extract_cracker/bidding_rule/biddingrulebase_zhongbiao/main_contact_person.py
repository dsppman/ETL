...
from bidding_rb.pipeline_contact_person import Pipeline
...
def run_pipeline(data_path):
    ...
    for data in tqdm(datas):
        ...
        try:
            ...
            data['中标单位联系人'] = list({i['text'] for i in result})
            data['source'] = list({i['source'] for i in result})
            # print('处理后的中标联系人: %s ' % data['中标联系人'])
...
def evaluate(true_data_path, pred_data_path):
    ...
    for url in url2true_data.keys():
        ...
        y_true = set(true_data.get('中标单位联系人', []))
        y_pred = set(pred_data.get('中标单位联系人', []))
    ...
    # metrics, miss, error = calc_metrics(true_set, pred_set)
    # print(metrics)
    metrics_df = pd.DataFrame([metrics], index=['中标单位联系人'])
