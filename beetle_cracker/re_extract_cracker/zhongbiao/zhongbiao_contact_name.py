...
from beetle_cracker.re_extract_cracker.common.rules.data_owner import target_keywords, contact_person_words, \
    contact_number_words, rule_supplement_words, family_name_words
        ...
        for target_keyword in self.target_keywords:
            ...
            if flag:
                for match_result in match_results:
                    ...
                    regex = r'招\s*标\s*人|招\s*标\s*机\s*构|采\s*购\s*人|采\s*购\s*机\s*构|甲\s*方|采\s*购\s*单\s*位'
...
def get_data(tables, text, re_compiled_set):
    ...
    res_dict = {"type": "zhongbiao_contact_name", "data": data}
    return res_dict
